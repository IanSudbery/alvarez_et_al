...
# condition and batch.
# 1) Starting from the raw counts specified (all regions), performs Rlog with blind = TRUE. On the Rlog results, 
...
counts_matrix_file = "tag_counts.dir/pan_raw_tag_counts.tech_reps_collapsed.tsv.gz"
coldata_file = "tag_counts.dir/pan_raw_tag_counts.tech_reps_collapsed.col_data.tsv"
output_file = "rlog.dir/atac_pan_peaks_subgroup_correction_rlog.tsv.gz"
...
# remove cell lines
coldata_df <- subset(coldata_df, MM.ND != "Cell_line")
input_matrix <- input_matrix[,row.names(coldata_df)]

# rename ND to PC for backwards compatability
coldata_df$Subgroup[coldata_df$Subgroup == "ND"] <- "PC"
