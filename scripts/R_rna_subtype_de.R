...
# condition and batch.
# 1) Performs a Log Ratio Test for condition accounting for batch.
# 2) Outputs the result from the LRT to a table (de_res_LRT_batch_condition_output_file).
# 3) Gets regions which are significant for any condition (padj<pdaj_threshold and padj not NA), note that padj is the same for any comparison below.
# 4) Gets significant results (3) for each condition differential expression comparison vs ND (padj<pdaj_threshold and padj not NA) and log2FoldChange > abs(very_significant_threshold).
# 5) Outputs for any condition vs ND differential expression (4):
...
#     -DE_subgroup_vs_ND: Whether the subgroup is Differentially expressed with respect to ND in terms of the log2FoldChange:
...
# 7) Add the cytogenetics groups to the sample names on the table (6) 
...
# 
# Inputs:
#   -very_significant_threshold: Only regions where the log2FoldChange of subgroup vs ND >= than
#       abs(this) will be outputted to the significant
#       subgroup regions (For example, sign_LRT_batch_condition_HD_vs_ND_unique_file)
...
#
#
#   -de_res_LRT_batch_condition_output_file: A matrix with the results from LRT.
#   -A results file for each condition:
#     -sign_LRT_batch_condition_HD_vs_ND_unique_file.
#     -sign_LRT_batch_condition_CCND1_vs_ND_unique_file.
#     -etc...
#
...
de_res_LRT_batch_condition_output_file = "DE.dir/all_subtype_rna.tsv.gz"
sign_LRT_batch_condition_any_condition_file = "DE.dir/sign_subtype_rna.tsv.gz"
output_heatmaps = "DE.dir/sign_subtype_rna.png"
...
# Check all parameters are not empty
if(de_res_LRT_batch_condition_output_file == '' || is.null(de_res_LRT_batch_condition_output_file)){
  stop("ERROR: de_res_LRT_batch_condition_output_file is empty")
}
# Check all parameters are not empty
if(sign_LRT_batch_condition_any_condition_file == '' || is.null(sign_LRT_batch_condition_any_condition_file)){
  stop("ERROR: sign_LRT_batch_condition_any_condition_file is empty")
}
...
# Get the result.
res_LRT_batch_condition=as.data.frame(results(dds.LRT))

# setDT modifies the input, make a copy of the dataframe
res_LRT_batch_condition_ori <- data.frame(res_LRT_batch_condition)


# Output the different differential results
# Convert row names into first column
output_df_res_LRT_batch_condition = setDT(res_LRT_batch_condition_ori, keep.rownames = TRUE)[]

# Remove the variable
remove(res_LRT_batch_condition_ori)


# Rename the id column
colnames(output_df_res_LRT_batch_condition)[1] <- first_col_name


# Output the file
write.table(output_df_res_LRT_batch_condition, gzfile(de_res_LRT_batch_condition_output_file),
            append = FALSE,
            quote = FALSE,
            sep = "\t",
            eol = "\n",
...
# We get anything which is significant for any condition
sign_LRT_batch_condition = res_LRT_batch_condition[(!(is.na(res_LRT_batch_condition$padj))&(res_LRT_batch_condition$padj<pdaj_threshold)),]

# Now we compare the different cell lines to ND
# resultsNames(dds.LRT)



res_HD_vs_ND=as.data.frame(results(dds.LRT, name="condition_HD_vs_ND"))
...
# reduced=~ batch
# is significant.
# We get the log2FoldChange changes above 1 (above 2) with p-adj < pdaj_threshold

# Get the very significant results
very_sign_LRT_batch_condition_HD_vs_ND = res_HD_vs_ND[(!(is.na(res_HD_vs_ND$padj))&(res_HD_vs_ND$padj<pdaj_threshold)&(abs(res_HD_vs_ND$log2FoldChange)>=very_significant_threshold)),]



res_CCND1_vs_ND=as.data.frame(results(dds.LRT, name="condition_CCND1_vs_ND"))

# Get the very significant results
very_sign_LRT_batch_condition_CCND1_vs_ND = res_CCND1_vs_ND[(!(is.na(res_CCND1_vs_ND$padj))&(res_CCND1_vs_ND$padj<pdaj_threshold)&(abs(res_CCND1_vs_ND$log2FoldChange)>=very_significant_threshold)),]





res_MAF_vs_ND=as.data.frame(results(dds.LRT, name="condition_MAF_vs_ND"))

# Get the very significant results
very_sign_LRT_batch_condition_MAF_vs_ND = res_MAF_vs_ND[(!(is.na(res_MAF_vs_ND$padj))&(res_MAF_vs_ND$padj<pdaj_threshold)&(abs(res_MAF_vs_ND$log2FoldChange)>=very_significant_threshold)),]


res_MMSET_vs_ND=as.data.frame(results(dds.LRT, name="condition_MMSET_vs_ND"))

# Get the very significant results
very_sign_LRT_batch_condition_MMSET_vs_ND = res_MMSET_vs_ND[(!(is.na(res_MMSET_vs_ND$padj))&(res_MMSET_vs_ND$padj<pdaj_threshold)&(abs(res_MMSET_vs_ND$log2FoldChange)>=very_significant_threshold)),]



# Get the rownames for each very significant result and concatenate them into one list
all_very_sign_regions = rownames(very_sign_LRT_batch_condition_HD_vs_ND)
all_very_sign_regions = c(all_very_sign_regions, rownames(very_sign_LRT_batch_condition_CCND1_vs_ND))
all_very_sign_regions = c(all_very_sign_regions, rownames(very_sign_LRT_batch_condition_MAF_vs_ND))
all_very_sign_regions = c(all_very_sign_regions, rownames(very_sign_LRT_batch_condition_MMSET_vs_ND))


# Get unique regions
unique_very_sign_regions = unique(all_very_sign_regions)

# Get a table with the general details (LRT) for the very significant results (in any subgroup)
df_very_sign_res_any_subgroup = res_LRT_batch_condition[unique_very_sign_regions,c("baseMean","pvalue", "padj")]




# Now we have to add the individual details for each subgroup comparison in the table
df_very_sign_res_any_subgroup[,"log2FoldChange_HD_vs_ND"] = res_HD_vs_ND[unique_very_sign_regions,"log2FoldChange"]
df_very_sign_res_any_subgroup[,"lfcSE_HD_vs_ND"] = res_HD_vs_ND[unique_very_sign_regions,"lfcSE"]


df_very_sign_res_any_subgroup[,"log2FoldChange_CCND1_vs_ND"] = res_CCND1_vs_ND[unique_very_sign_regions,"log2FoldChange"]
df_very_sign_res_any_subgroup[,"lfcSE_CCND1_vs_ND"] = res_CCND1_vs_ND[unique_very_sign_regions,"lfcSE"]


df_very_sign_res_any_subgroup[,"log2FoldChange_MAF_vs_ND"] = res_MAF_vs_ND[unique_very_sign_regions,"log2FoldChange"]
df_very_sign_res_any_subgroup[,"lfcSE_MAF_vs_ND"] = res_MAF_vs_ND[unique_very_sign_regions,"lfcSE"]


df_very_sign_res_any_subgroup[,"log2FoldChange_MMSET_vs_ND"] = res_MMSET_vs_ND[unique_very_sign_regions,"log2FoldChange"]
df_very_sign_res_any_subgroup[,"lfcSE_MMSET_vs_ND"] = res_MMSET_vs_ND[unique_very_sign_regions,"lfcSE"]
...
df_very_sign_res_any_subgroup[,"DE_HD_vs_ND"] = 0
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_HD_vs_ND<=-very_significant_threshold,"DE_HD_vs_ND"] = -1
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_HD_vs_ND>=very_significant_threshold,"DE_HD_vs_ND"] = 1


df_very_sign_res_any_subgroup[,"DE_CCND1_vs_ND"] = 0
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_CCND1_vs_ND<=-very_significant_threshold,"DE_CCND1_vs_ND"] = -1
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_CCND1_vs_ND>=very_significant_threshold,"DE_CCND1_vs_ND"] = 1


df_very_sign_res_any_subgroup[,"DE_MAF_vs_ND"] = 0
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_MAF_vs_ND<=-very_significant_threshold,"DE_MAF_vs_ND"] = -1
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_MAF_vs_ND>=very_significant_threshold,"DE_MAF_vs_ND"] = 1


df_very_sign_res_any_subgroup[,"DE_MMSET_vs_ND"] = 0
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_MMSET_vs_ND<=-very_significant_threshold,"DE_MMSET_vs_ND"] = -1
df_very_sign_res_any_subgroup[df_very_sign_res_any_subgroup$log2FoldChange_MMSET_vs_ND>=very_significant_threshold,"DE_MMSET_vs_ND"] = 1
...
# setDT modifies the input, make a copy of the dataframe
df_very_sign_res_any_subgroup_ori <- data.frame(df_very_sign_res_any_subgroup)


# Output the different differential results
# Convert row names into first column
output_df_very_sign_res_any_subgroup = setDT(df_very_sign_res_any_subgroup_ori, keep.rownames = TRUE)[]

# Remove the variable
remove(df_very_sign_res_any_subgroup_ori)


# Rename the id column
colnames(output_df_very_sign_res_any_subgroup)[1] <- first_col_name


# Output the file
write.table(output_df_very_sign_res_any_subgroup, 
            gzfile(sign_LRT_batch_condition_any_condition_file),
            append = FALSE,
            quote = FALSE,
            sep = "\t",
            eol = "\n",
